...
namespace
{
	CryptoPP::AutoSeededRandomPool* g_pUdpCryptRandomGen = NULL;

	CryptoPP::AutoSeededRandomPool& GetUdpCryptRandomGen()
	{
		if (g_pUdpCryptRandomGen == NULL)
			g_pUdpCryptRandomGen = new CryptoPP::AutoSeededRandomPool();
		return *g_pUdpCryptRandomGen;
	}
}

void FreeEncryptedDatagramSocketRandomPool()
{
	delete g_pUdpCryptRandomGen;
	g_pUdpCryptRandomGen = NULL;
}

int CEncryptedDatagramSocket::DecryptReceivedClient(BYTE *pbyBufIn, int nBufLen, BYTE **ppbyBufOut, const CAddress& dwIP, uint32 *nReceiverVerifyKey, uint32 *nSenderVerifyKey)
{
	int nResult = nBufLen;
	*ppbyBufOut = pbyBufIn;
	...
	*nSenderVerifyKey = 0;
	...
	do {
		...
		case 1: // ed2k packet
			...
			bKadRecvKeyUsed = false;
			{
				...
				if (dwIP.GetType() == CAddress::IPv6) {
					...
					len = 23;
					uint32 uIP = dwIP.ToUInt32(false);
		...
		case 2: // kad packet with ReceiverKey as key
			...
			if (Kademlia::CKademlia::GetPrefs()) {
				uchar achKeyData[6];
				PokeUInt32(achKeyData, Kademlia::CPrefs::GetUDPVerifyKey(dwIP.ToUInt32(false)));
	...
	if (dwValue == MAGICVALUE_UDP_SYNC_CLIENT) {
		...
		*ppbyBufOut = &pbyBufIn[nBufLen - nResult];
		...
		DebugLogWarning(_T("Obfuscated packet expected but magicvalue mismatch on UDP packet from clientIP: %s, Possible RecvKey: %u"), (LPCTSTR)ipstr(dwIP), Kademlia::CPrefs::GetUDPVerifyKey(dwIP.ToUInt32(false)));
	return nResult;
}

// Encrypt packet. Key used:
uint32 CEncryptedDatagramSocket::EncryptSendClient(uchar *pbyBuf, uint32 nBufLen, const uchar *pachClientHashOrKadID, bool bKad, uint32 nReceiverVerifyKey, uint32 nSenderVerifyKey, bool bIPv6)
{
	ASSERT(theApp.GetPublicIPv4() != 0 || bKad);
	...
	const uint16 nRandomKeyPart = (uint16)GetUdpCryptRandomGen().GenerateWord32(0, _UI16_MAX);
	MD5Sum md5;
	if (bKad) {
		...
		if (bIPv6 && !theApp.GetPublicIPv6().IsNull())
		{
			...
			len = 23;
			uint32 dwIP = theApp.GetPublicIPv4();
	...
	for (int i = 32; i > 0; --i) {
		bySemiRandomNotProtocolMarker = GetUdpCryptRandomGen().GenerateByte();
		if (bKad) {
