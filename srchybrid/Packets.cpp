...
{
	ASSERT(completebuffer == NULL);
	// If we own a buffer (allocated in ctor), free it. AttachPacket/DetachPacket manage ownership explicitly: DetachPacket() transfers ownership to caller and sets pBuffer to NULL.
	if (pBuffer) {
		delete[] pBuffer;
		pBuffer = NULL;
	}
...
bool CTag::WriteTagToFile(CFileDataIO &file, EUTF8str eStrEncode) const
{
	...
	if (IsHash() || IsStr() || IsInt64() || IsFloat() || IsBlob()) {
