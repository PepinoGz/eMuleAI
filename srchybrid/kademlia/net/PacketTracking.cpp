...
int CPacketTracking::InTrackListIsAllowedPacket(uint32 uIP, uint8 byOpcode, bool /*bValidSenderkey*/)
{
	...
	if (i >= 0) {
		...
		if (thePrefs.IsDetectKadRequestFlooder()) {
		  // now the actual check if this request is allowed
		  if (TrackedRequest.m_tokens < 0) {
			  if (TrackedRequest.m_tokens < MIN2MS(-(thePrefs.GetKadRequestFloodBanTreshold()-1))) { //m_tokens < 0 means that we already already spent 1 minute tokens, so -1 used here
				  ...
				  AddProtectionLogLine(false, _T("Kad: Massive request flood detected for opcode 0x%X (0x%X) from IP %s - Banning IP"), byOpcode, byDbgOrgOpcode, (LPCTSTR)ipstr(htonl(uIP)));
				  theApp.clientlist->AddBannedClient(ipstr(ntohl(uIP)));
			  ...
			  if (!TrackedRequest.m_bDbgLogged) {
				  ...
				  AddProtectionLogLine(false, _T("Kad: Request flood detected for opcode 0x%X (0x%X) from IP %s - Dropping packets with this opcode"), byOpcode, byDbgOrgOpcode, (LPCTSTR)ipstr(htonl(uIP)));
			  }
