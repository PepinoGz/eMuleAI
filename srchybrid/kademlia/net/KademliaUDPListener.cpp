...
void CKademliaUDPListener::FirewalledCheck(uint32 uIP, uint16 uUDPPort, const CKadUDPKey &senderUDPKey, uint8 byKadVersion)
{
	...
	theApp.clientlist->AddKadFirewallRequest(CAddress(uIP, true));
}
...
void CKademliaUDPListener::ProcessPacket(const byte *pbyData, uint32 uLenData, uint32 uIP, uint16 uUDPPort, bool bValidReceiverKey, const CKadUDPKey &senderUDPKey)
{
	...
	case KADEMLIA_FIREWALLED_ACK_RES:
		...
		break;
	case KADEMLIA_FINDSERVINGBUDDY_REQ:
		if (thePrefs.GetDebugClientKadUDPLevel() > 0)
			DebugRecv("KADEMLIA_FINDSERVINGBUDDY_REQ", uIP, uUDPPort);
		Process_KADEMLIA_FINDSERVINGBUDDY_REQ(pbyPacketData, uLenData, uIP, uUDPPort, senderUDPKey);
		break;
	case KADEMLIA_FINDSERVINGBUDDY_RES:
		if (thePrefs.GetDebugClientKadUDPLevel() > 0)
			DebugRecv("KADEMLIA_FINDSERVINGBUDDY_RES", uIP, uUDPPort);
		Process_KADEMLIA_FINDSERVINGBUDDY_RES(pbyPacketData, uLenData, uIP, uUDPPort, senderUDPKey);
		break;
...
// Process ICMP unreachable events
void CKademliaUDPListener::ProcessICMPUnreachable(uint32 uIP, uint16 uUDPPort)
{
	// Force increment of contact type in routing table, to speed up removal of this unreliable contact
	CContact* contact = CKademlia::GetRoutingZone()->GetContact(uIP, uUDPPort, false);
	if (contact)
		contact->CheckingType();

	// Inform searchmanager about the unreachable contact if this was a response to a search request
	if (IsOnOutTrackList(uIP, KADEMLIA2_REQ, true))
		CSearchManager::ProcessUnreachableResponse(uIP, uUDPPort);
}
...
void CKademliaUDPListener::Process_KADEMLIA2_HELLO_RES_ACK(const byte *pbyPacketData, uint32 uLenPacket, uint32 uIP, uint16 uUDPPort, bool bValidReceiverKey)
{
	...
	safeKad.TrackNode(uIP, uUDPPort, uRemoteID, true); // Remember this so we can ignore any other identities for this node
...
void CKademliaUDPListener::Process_KADEMLIA2_HELLO_RES(const byte *pbyPacketData, uint32 uLenPacket, uint32 uIP, uint16 uUDPPort, const CKadUDPKey &senderUDPKey, bool bValidReceiverKey)
{
	...
	if (bSendACK) {
		...
		if (senderUDPKey.IsEmpty()) {
			...
			safeKad.TrackNode(uIP, uUDPPort, uContactID, true); // Remember this so we can ignore any other identities for this node
			...
			if (uVersion >= KADEMLIA_VERSION2_47a) { // Kad1 nodes are no longer accepted, and are ignored
				if (IsGoodIPPort(uhostIPResult, uUDPPortResult)) {
					if (!theApp.ipfilter->IsFiltered(uhostIPResult) && !(uUDPPortResult == 53 && uVersion <= KADEMLIA_VERSION5_48a)  /*No DNS Port without encryption*/) {
						...
						AddDebugLogLine(false, _T("Ignored kad contact (IP=%s:%u) - IP filter (%s)"), (LPCTSTR)ipstr(uhostIPResult), uUDPPortResult, (LPCTSTR)EscPercent(theApp.ipfilter->GetLastHit()));
		...
		for (unsigned uTags = byteIO.ReadByte(); uTags > 0; --uTags) {
			...
			if (pTag) {
				if (!pTag->m_name.Compare(TAG_SOURCETYPE)) {
					...
					//drop lowID sources with unreachable serving buddy (Enig123)
					if (pTag->IsInt()) {
						LPCTSTR p = NULL;
						uint32 uServingBuddyIP = (uint32)pTag->GetInt();
						//} else
						if (theApp.ipfilter->IsFiltered(uServingBuddyIP)) {
							if (thePrefs.GetLogFilteredIPs())
								p = _T("IP-filtered");
						} else if (theApp.clientlist->IsBannedClient(ipstr(uServingBuddyIP))) {
						...
						if (pTag) {
							pEntry->m_bSource = false;
							if (p)
								AddDebugLogLine(false, _T("Publish request from source %s with %s serving buddy IP=%s"), (LPCTSTR)ipstr(htonl(uIP)), p, (LPCTSTR)ipstr(uServingBuddyIP));
						}
					}
				} else {
					//TODO: Filter tags
...
void CKademliaUDPListener::Process_KADEMLIA_FIREWALLED_RES(const byte *pbyPacketData, uint32 uLenPacket, uint32 uIP, const CKadUDPKey& /*senderUDPKey*/)
{
	...
	if (!theApp.clientlist->IsKadFirewallCheckIP(CAddress(uIP, true))) { /*KADEMLIA_FIREWALLED2_REQ + KADEMLIA_FIREWALLED_REQ*/
		CString strError;
...
// Used by Kad1.0 and Kad2.0
void CKademliaUDPListener::Process_KADEMLIA_FINDSERVINGBUDDY_REQ(const byte *pbyPacketData, uint32 uLenPacket, uint32 uIP, uint16 uUDPPort, const CKadUDPKey &senderUDPKey)
{
	// Verify packet is expected size
	if (uLenPacket < 34) {
		CString strError;
	...
	if (CKademlia::GetPrefs()->GetFirewalled() || CUDPFirewallTester::IsFirewalledUDP(true) || !CUDPFirewallTester::IsVerified()) {
		...
		DebugLog(_T("[Buddy]: Reject serving buddy req from %s:%hu (unverified or firewalled).\n"), (LPCTSTR)ipstr(htonl(uIP)), uUDPPort);
		return;
	}
	...
	CSafeMemFile fileIO(pbyPacketData, uLenPacket);
	CUInt128 ServedBuddyID;
	fileIO.ReadUInt128(ServedBuddyID);
	CUInt128 userID;
	fileIO.ReadUInt128(userID);
	uint16 uTCPPort = fileIO.ReadUInt16();

	// Optional: Requester's connect options. Older version clientd won't send this byte.
	uint8 byReqConnectOpts = 0;
	if (uLenPacket > 34)
		byReqConnectOpts = fileIO.ReadUInt8();
	...
	if (!theApp.clientlist->IncomingServedBuddy(&contact, &ServedBuddyID))
		return; // cancelled for some reason, don't send a response

	// Register served buddy so we can handle pings and callbacks.
	// Try to locate the newly created client by IP:TCP
	CUpDownClient* pNew = theApp.clientlist->FindClientByIP(CAddress(uIP, true), contact.GetTCPPort());
	if (pNew != NULL)
	...
	fileIO2.WriteUInt16(thePrefs.GetPort()); // Serving buddy response carries TCP port for buddy handshake connection.
	...
	if (thePrefs.GetDebugClientKadUDPLevel() > 0)
		DebugSend("KADEMLIA_FINDSERVINGBUDDY_RES", uIP, uUDPPort);

	SendPacket(fileIO2, KADEMLIA_FINDSERVINGBUDDY_RES, uIP, uUDPPort, senderUDPKey, NULL);
}

// Used by Kad1.0 and Kad2.0
void CKademliaUDPListener::Process_KADEMLIA_FINDSERVINGBUDDY_RES(const byte *pbyPacketData, uint32 uLenPacket, uint32 uIP, uint16 uUDPPort, const CKadUDPKey& /*senderUDPKey*/)
{
	// Verify packet is expected size
	if (uLenPacket < 34) {
		CString strError;
	...
	if (!IsOnOutTrackList(uIP, KADEMLIA_FINDSERVINGBUDDY_REQ)) {
		CString strError;
		strError.Format(_T("***NOTE: Received unrequested response packet, size (%u) in %hs"), uLenPacket, __FUNCTION__);
		throw strError;
	}
...
void CKademliaUDPListener::Process_KADEMLIA_CALLBACK_REQ(const byte* pbyPacketData, uint32 uLenPacket, uint32 uIP, const CKadUDPKey& /*senderUDPKey*/)
{
	...
	// Select served buddy by KadID first, then fall back to serving buddy.
	CSafeMemFile fileIO(pbyPacketData, uLenPacket);
	CUInt128 uCheck;
	fileIO.ReadUInt128(uCheck);
	// JOHNTODO: Begin filtering bad serving buddy ID's.
	CUInt128 uFile;
	fileIO.ReadUInt128(uFile);
	uint16 uTCP = fileIO.ReadUInt16();

	// Requester-provided public IPv4 (host order)
	uint32 extIP = 0;
	if (fileIO.GetLength() - fileIO.GetPosition() >= 4)
		extIP = fileIO.ReadUInt32();

	CUpDownClient* pTarget = theApp.clientlist->FindServedBuddyByKadID(uCheck);
	if (pTarget == NULL)
		pTarget = theApp.clientlist->GetServingBuddy();
	...
	if (pTarget != NULL) {
		...
		// Prefer requester-declared public IP if source is private IP. OP_CALLBACK expects IPv4 in host byte order
		uint32 outIP = uIP; // uIP is host order in this context
		if (!CAddress(outIP, true).IsPublicIP() && extIP != 0 && CAddress(extIP, true).IsPublicIP())
			outIP = extIP; // extIP already host order
		fileIO2.WriteUInt32(outIP);
		pTarget->SetLastCallbackRequesterIP(CAddress(outIP, false)); // Remember requester endpoint for upcoming OP_REASKCALLBACKUDP forwards
		...
		if (thePrefs.GetDebugClientKadUDPLevel() > 0 || thePrefs.GetDebugClientTCPLevel() > 0)
			DebugSend("OP_CALLBACK", pTarget);
		theStats.AddUpDataOverheadFileRequest(pPacket->size);
		pTarget->socket->SendPacket(pPacket);
	}
}
...
void CKademliaUDPListener::SendPacket(const byte *pbyData, uint32 uLenData, uint32 uDestinationHost, uint16 uDestinationPort, const CKadUDPKey &targetUDPKey, const CUInt128 *uCryptTargetID)
{
	...
	if (theApp.clientudp != NULL) {
		theApp.clientudp->SendPacket(pPacket, CAddress(uDestinationHost, true), uDestinationPort, true
			, (uCryptTargetID != NULL) ? uCryptTargetID->GetData() : NULL
			, true, targetUDPKey.GetKeyValue(theApp.GetPublicIPv4()));
	} else {
		// If there is no UDP socket yet, avoid leaking the packet
		delete pPacket;
	}
}
