...
namespace Kademlia
{
	...
	public:
		...
		void ProcessICMPUnreachable(uint32 uIP, uint16 uUDPPort); // Process ICMP unreachable events
	...
	private:
		...
		void Process_KADEMLIA_FINDSERVINGBUDDY_REQ(const byte *pbyPacketData, uint32 uLenPacket, uint32 uIP, uint16 uUDPPort, const CKadUDPKey &senderUDPKey);
		void Process_KADEMLIA_FINDSERVINGBUDDY_RES(const byte *pbyPacketData, uint32 uLenPacket, uint32 uIP, uint16 uUDPPort, const CKadUDPKey &senderUDPKey);
