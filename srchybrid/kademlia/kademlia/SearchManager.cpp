...
void CSearchManager::StopAllSearches()
{
	...
	for (SearchMap::const_iterator itSearchMap = m_mapSearches.begin(); itSearchMap != m_mapSearches.end(); ++itSearchMap)
		delete itSearchMap->second;
	g_pKadSearchMap->clear();
	delete g_pKadSearchMap;
...
CSearch* CSearchManager::PrepareFindKeywords(LPCWSTR szKeyword, UINT uSearchTermsSize, LPBYTE pucSearchTermsData)
{
	...
	try {
		...
		if (pSearch->m_listWords.empty())
			strError = GetResString(_T("KAD_SEARCH_KEYWORD_TOO_SHORT"));
		else {
			...
			if (AlreadySearchingFor(pSearch->m_uTarget))
				strError.Format(GetResString(_T("KAD_SEARCH_KEYWORD_ALREADY_SEARCHING")), (LPCTSTR)wstrKeyword);
			else {
...
// Find the amount of contacts this search will accept
uint8 CSearchManager::GetRequestContactCountFor(const CUInt128& uTarget)
{
...
void CSearchManager::JumpStart()
{
	...
	for (SearchMap::const_iterator itSearchMap = m_mapSearches.begin(); itSearchMap != m_mapSearches.end();) {
		...
		case CSearch::FINDSERVINGBUDDY:
			if (tNow >= pSearch->m_tCreated + SEARCHFINDSERVINGBUDDY_LIFETIME)
				bDel = true;
			else if (pSearch->GetAnswers() >= SEARCHFINDSERVINGBUDDY_TOTAL || tNow >= pSearch->m_tCreated + SEARCHFINDSERVINGBUDDY_LIFETIME - SEC(20))
				bStop = true;
			break;
...
void CSearchManager::ProcessUnreachableResponse(uint32 uFromIP, uint16 uFromPort)
{
	// Inform all searches.
	for (SearchMap::iterator itSearchMap = m_mapSearches.begin(); itSearchMap != m_mapSearches.end(); ++itSearchMap)
	{
		CSearch* pSearch = itSearchMap->second;
		if (pSearch != NULL)
			pSearch->ProcessUnreachableResponse(uFromIP, uFromPort);
	}
}
...
void CSearchManager::ProcessInvalidResponse(const CUInt128& uTarget, uint32 uFromIP, uint16 uFromPort)
{
	// We got a response to a kad lookup.
	SearchMap::const_iterator itSearchMap = m_mapSearches.find(uTarget);
	if (itSearchMap != m_mapSearches.end())
		itSearchMap->second->ProcessInvalidResponse(uFromIP, uFromPort);
}
