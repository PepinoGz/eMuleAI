...
time_t		CKademlia::m_tNextFindServingBuddy;
...
void CKademlia::Start(CPrefs *pPrefs)
{
	try {
		...
		m_clkNextSearchJumpStart = clock();
...
void CKademlia::Stop()
{
	...
	if (g_pKadEventMap != NULL) {
		g_pKadEventMap->clear();
		delete g_pKadEventMap;
		g_pKadEventMap = NULL;
	}
}
...
void CKademlia::Process()
{
	...
	if (tNow >= m_tNextFindServingBuddy) {
		m_pInstance->m_pPrefs->SetFindServingBuddy();
		m_tNextFindServingBuddy = tNow + MIN2S(20);
	}
	...
	// Moved to UploadQueue, so it runs at every cycle!!!
	...
	if (!IsConnected() && (tNow >= m_tBootstrap + 15 || (GetRoutingZone()->GetNumContacts() == 0 && tNow >= m_tBootstrap + 2)))
		if (!s_liBootstrapList.IsEmpty()) {
			...
			// failed to bootstrap
			m_bootstrapping = false;
			AddLogLine(true, GetResString(_T("BOOTSTRAPFAILED")));
		}
...
void CKademlia::ProcessICMPUnreachable(uint32 uIP, uint16 uPort)
{
	if (m_pInstance && m_pInstance->m_pUDPListener)
		m_pInstance->m_pUDPListener->ProcessICMPUnreachable(uIP, uPort);
}
...
void CKademlia::RecheckFirewalled()
{
	if (m_pInstance && m_pInstance->GetPrefs() && !IsRunningInLANMode()) {
		...
		// Stop any new serving buddy requests, and tell the client
		// to recheck it's IP which in turns rechecks firewall.
		m_pInstance->m_pPrefs->SetFindServingBuddy(false);
		m_pInstance->m_pPrefs->SetRecheckIP();
		// also UDP check
		CUDPFirewallTester::ReCheckFirewallUDP(false);

		time_t tNow = time(NULL);
		// Delay the next serving buddy search to at least 5 minutes after our firewall check so we are sure to be still firewalled
		if (m_tNextFindServingBuddy < tNow + MIN2S(5))
			m_tNextFindServingBuddy = tNow + MIN2S(5);
		m_tNextFirewallCheck = tNow + HR2S(1);
		m_tNextUPnPCheck = m_tNextFirewallCheck - MIN2S(1);
	}
...
CKademliaUDPListener* CKademlia::TryGetUDPListener()
{
	if (m_pInstance == NULL)
		return NULL;
	return m_pInstance->m_pUDPListener;
...
CRoutingZone* CKademlia::TryGetRoutingZone()
{
	if (m_pInstance == NULL)
		return NULL;
	return m_pInstance->m_pRoutingZone;
