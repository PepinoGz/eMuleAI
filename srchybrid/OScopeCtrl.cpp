...
void COScopeCtrl::InvalidateCtrl(bool deleteGraph)
{
	...
	if (bStraightGraphs) {
		...
		crLabelBk = GetCustomSysColor(hbr == GetSysColorBrush(COLOR_WINDOW) ? COLOR_WINDOW : COLOR_BTNFACE);
		crLabelFg = GetCustomSysColor(COLOR_WINDOWTEXT);
	} else {
...
void COScopeCtrl::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if ((nFlags & MK_LBUTTON) == 0) {
		if (m_uLastMouseFlags & MK_LBUTTON) {
			...
			CWnd *pwndParent = GetParent();
			if (pwndParent)
				pwndParent->SendMessage(UM_OSCOPEPOSITION, 0, (LPARAM)(LPCTSTR)EMPTY);
		}
	...
	if (pwndParent) {
		...
		strInfo.AppendFormat(_T(": %u @ %s ") + GetResString(_T("TIMEBEFORE")), yValue, szDate, (LPCTSTR)CastSecondsToLngHM(dwTime));
