...
void CEncryptedStreamSocket::ResetStreamCryptLayer()
{
	if (m_pRC4ReceiveKey != NULL) {
		delete m_pRC4ReceiveKey;
		m_pRC4ReceiveKey = NULL;
	}
	if (m_pRC4SendKey != NULL) {
		delete m_pRC4SendKey;
		m_pRC4SendKey = NULL;
	}
	if (m_pfiReceiveBuffer != NULL) {
...
void CEncryptedStreamSocket::CryptPrepareSendData(uchar *pBuffer, uint32 nLen)
{
	if (!IsEncryptionLayerReady()) {
		if (IsUtpBypassSocket(this))
			return; // uTP handles its own encryption; safe to proceed without transforming buffer
