...
CKnownFile::~CKnownFile()
{
		...
		if (theApp.knownfiles)
			theApp.knownfiles->PurgeDuplicateFile(this);
...
void CKnownFile::UpdatePartsInfo()
{
	...
	if (bRefresh)
		acount.reserve(m_ClientUploadList.GetCount());
	...
	if (bRefresh) {
		...
		acount.push_back(m_nCompleteSourcesCount + 1); // plus 1 since we have the complete file too
...
bool CKnownFile::CreateFromFile(LPCTSTR in_directory, LPCTSTR in_filename, LPVOID pvProgressParam)
{
	...
	strFilePathTmp = in_directory;
	if (!strFilePathTmp.IsEmpty() && strFilePathTmp[strFilePathTmp.GetLength() - 1] != _T('\\')) 
		strFilePathTmp += _T('\\');
	...
	if ((uint64)llFileSize > MAX_EMULE_FILE_SIZE) {
		if (llFileSize <= 0)
			LogError(_T("Failed to hash file \"%s\" - %s"), (LPCTSTR)EscPercent(strFilePath), (LPCTSTR)EscPercent(_tcserror(errno)));
		else
			LogError(_T("Skipped hashing file \"%s\" - File size exceeds limit."), (LPCTSTR)EscPercent(strFilePath));
		fclose(file);
	...
	for (hashcount = 0; ; ++hashcount) {
		...
		if (pvProgressParam) {
			if (theApp.IsClosing()) {
				LogError(_T("Hashing cancelled (closing eMule), file \"%s\""), (LPCTSTR)EscPercent(strFilePath));
				fclose(file);
				return false;
			}
			...
			{
				LogError(_T("Hashing cancelled (pending delete), file \"%s\""), (LPCTSTR)EscPercent(strFilePath));
				fclose(file);
				return false;
			}
			ASSERT(reinterpret_cast<CKnownFile*>(pvProgressParam)->IsKindOf(RUNTIME_CLASS(CKnownFile)));

			CKnownFile* pProgFile = reinterpret_cast<CKnownFile*>(pvProgressParam);
			if (pProgFile->GetFileSize() != GetFileSize() && pProgFile->IsKindOf(RUNTIME_CLASS(CPartFile)))
				static_cast<CPartFile*>(pProgFile)->SetFileSize(GetFileSize());

			WPARAM uProgress = (WPARAM)((((uint64)GetFileSize() - togo) * 100) / ((uint64)GetFileSize() ? (uint64)GetFileSize() : (uint64)1));
	...
	if (cAICHHashSet.VerifyHashTree(true)) {
		...
		if (!m_FileIdentifier.SetAICHHashSet(cAICHHashSet)) {
			ASSERT(0);
			DebugLogError(_T("CreateFromFile() - failed to create AICH PartHashSet out of RecoveryHashSet - %s"), (LPCTSTR)EscPercent(GetFileName()));
		}
		...
		DebugLogError(LOG_STATUSBAR, _T("Failed to calculate AICH Hashset from file %s"), (LPCTSTR)EscPercent(GetFileName()));
...
bool CKnownFile::CreateAICHHashSetOnly()
{
	...
	for (UINT hashcount = 0; togo; ++hashcount) {
		...
		if (!CreateHash(file, uSize, NULL, pBlockAICHHashTree)) {
			LogError(_T("Failed to hash file \"%s\" - %s"), (LPCTSTR)EscPercent(GetFilePath()), (LPCTSTR)EscPercent(_tcserror(errno)));
			fclose(file);
			return false;
		}
		if (theApp.IsClosing()) { // in case of shutdown while still hashing
	...
	if (cAICHHashSet.VerifyHashTree(true)) {
		...
		if (!m_FileIdentifier.SetAICHHashSet(cAICHHashSet)) {
			ASSERT(0);
			DebugLogError(_T("CreateAICHHashSetOnly() - failed to create AICH PartHashSet out of RecoveryHashSet - %s"), (LPCTSTR)EscPercent(GetFileName()));
		}
		if (!cAICHHashSet.SaveHashSet())
			LogError(LOG_STATUSBAR, GetResString(_T("SAVEACFAILED")));
		else
...
bool CKnownFile::LoadTagsFromFile(CFileDataIO &file)
{
	...
	for (uint32 j = file.ReadUInt32(); j > 0; --j) {
		...
		case FT_FOLDERNAME:
			ASSERT(newtag->IsStr());
			if (newtag->IsStr() && GetPath().IsEmpty())
				SetPath(newtag->GetStr());
			break;
		case FT_FILEPATH:
			ASSERT(newtag->IsStr());
			if (newtag->IsStr() && GetFilePath().IsEmpty())
				SetFilePath(newtag->GetStr());
			break;
		case FT_FILESIZE:
...
bool CKnownFile::WriteToFile(CFileDataIO &file)
{
	...
	CTag pathtag(FT_FOLDERNAME, GetPath());
	pathtag.WriteTagToFile(file, UTF8strOptBOM);
	++uTagCount;

	CTag filepathtag(FT_FILEPATH, GetFilePath());
	filepathtag.WriteTagToFile(file, UTF8strOptBOM);
	++uTagCount;
...
void CKnownFile::CreateHash(CFile *pFile, uint64 Length, uchar *pMd4HashOut, CAICHHashTree *pShaHashOut)
{
	...
	for (uint64 Required = Length; Required;) {
		...
		VERIFY2(pFile->Read(X, uRead) == uRead);
...
Packet*	CKnownFile::CreateSrcInfoPacket(const CUpDownClient *forClient, uint8 byRequestedVersion, uint16 nRequestedOptions) const
{
	...
	if (!md4equ(forClient->GetUploadFileID(), GetFileHash())) {
		// should never happen
		DEBUG_ONLY(DebugLogError(_T("*** %hs - client (%s) upload file \"%s\" does not match file \"%s\""), __FUNCTION__, (LPCTSTR)EscPercent(forClient->DbgGetClientInfo()), (LPCTSTR)EscPercent(DbgGetFileInfo(forClient->GetUploadFileID())), (LPCTSTR)EscPercent(GetFileName())));
		ASSERT(0);
		return NULL;
	}
	...
	if (forClient->SupportsSourceExchange2() && byRequestedVersion > 0) {
		...
		if (forClient->SupportsExtendedSourceExchange()) {
			...
			if (thePrefs.GetLogExtendedSXEvents() && byUsedVersion == (uint8)SOURCEEXCHANGEEXT_VERSION)
				DebugLog(_T("[ExtendedSX] CKnownFile::CreateSrcInfoPacket: Client requested Extended Source Exchange, %s"), (LPCTSTR)EscPercent(forClient->DbgGetClientInfo()));
		} else
	...
	for (POSITION pos = m_ClientUploadList.GetHeadPosition(); pos != NULL;) {
		...
		if ((cur_src->HasLowID() && !cur_src->GetNatTraversalSupport()) || cur_src == forClient || !(cur_src->GetUploadState() == US_UPLOADING || cur_src->GetUploadState() == US_ONUPLOADQUEUE))
			continue;
		...
		if (rcvstatus) {
			...
			if (srcstatus) {
				...
				if (cur_src->GetUpPartCount() == forClient->GetUpPartCount()) {
					...
					// should never happen
					DEBUG_ONLY(DebugLogError(_T("*** %hs - found source (%s) with wrong part count (%u) attached to file \"%s\" (partcount=%u)"), __FUNCTION__, (LPCTSTR)EscPercent(cur_src->DbgGetClientInfo()), cur_src->GetUpPartCount(), (LPCTSTR)EscPercent(GetFileName()), GetPartCount()));
				}
			} else {
				++cDbgNoSrc;
		...
		if (bNeeded) {
			++nCount;
			uint32 dwID = (byUsedVersion >= 3 && !cur_src->HasLowID()) ? cur_src->GetUserIDHybrid() : ntohl(cur_src->GetUserIDHybrid());
	...
	TRACE2(_T("%hs: Out of %u clients, %u had no valid chunk status\n"), __FUNCTION__, m_ClientUploadList.GetCount(), cDbgNoSrc);
	...
	if (thePrefs.GetDebugSourceExchange())
		AddDebugLogLine(false, L"SXSend: Client source response extXS=%s, SX2=%s, Version=%u; Count=%u for %s, File=\"%s\"", forClient->SupportsExtendedSourceExchange() ? GetResString(_T("YES")) : GetResString(_T("NO")), bIsSX2Packet ? GetResString(_T("YES")) : GetResString(_T("NO")), byUsedVersion, nCount, (LPCTSTR)EscPercent(forClient->DbgGetClientInfo()), (LPCTSTR)EscPercent(GetFileName()));
	return result;
}
...
void CKnownFile::SetPublishedED2K(bool val)
{
	if (m_PublishedED2K == val)
		return; // No change, skip redundant PostMessage to avoid message queue flooding
	m_PublishedED2K = val;
	::PostMessage(theApp.emuledlg->GetSafeHwnd(), TM_SHAREDFILESCTRLUPDATEFILE, (WPARAM)this, 0);
}
...
bool CKnownFile::PublishSrc()
{
		...
		&& (Kademlia::CUDPFirewallTester::IsFirewalledUDP(true) || !Kademlia::CUDPFirewallTester::IsVerified()))
	{
		...
		if (!servingBuddy)
			return false;
		lastServingBuddyIP = servingBuddy->GetIP().ToUInt32(false);
		if (lastServingBuddyIP != m_lastServingBuddyIP) {
			SetLastPublishTimeKadSrc(tNow + KADEMLIAREPUBLISHTIMES, lastServingBuddyIP);
			return true;
		}
	} else
		lastServingBuddyIP = 0;
...
bool CKnownFile::ImportParts()
{
	...
	if (!IsPartFile()) {
		LogError(LOG_STATUSBAR, GetResString(_T("IMPORTPARTS_ERR_ALREADYCOMPLETE")));
		return false;
	}
...
CString CKnownFile::GetInfoSummary(bool bNoFormatCommands) const
{
	...
	if (strType.IsEmpty())
		strType += _T('-');
	const CString strAICHHash(m_FileIdentifier.HasAICHHash() ? m_FileIdentifier.GetAICHHash().GetString() : CString(_T("-")));
	CString dbgInfo;
#ifdef _DEBUG
	...
	info.AppendFormat(_T("\n")
		...
		, (LPCTSTR)GetResString(_T("PRIORITY")), (LPCTSTR)GetUpPriorityDisplayString()
		, (LPCTSTR)GetResString(_T("SF_REQUESTS")), (LPCTSTR)strRequests
		, (LPCTSTR)GetResString(_T("SF_ACCEPTS")), (LPCTSTR)strAccepts
		, (LPCTSTR)GetResString(_T("SF_TRANSFERRED")), (LPCTSTR)strTransferred, (LPCTSTR)dbgInfo
	);
	return info;
}
...
CString CKnownFile::GetUpPriorityDisplayString() const
{
	...
	case PR_VERYLOW:
		...
		break;
	case PR_LOW:
		uid = IsAutoUpPriority() ? _T("PRIOAUTOLOW") : _T("PRIOLOW");
		break;
	case PR_NORMAL:
		uid = IsAutoUpPriority() ? _T("PRIOAUTONORMAL") : _T("PRIONORMAL");
		break;
	case PR_HIGH:
		uid = IsAutoUpPriority() ? _T("PRIOAUTOHIGH") : _T("PRIOHIGH");
		break;
	case PR_VERYHIGH:
		uid = _T("PRIORELEASE");
		break;
...
bool CKnownFile::ShouldPartiallyPurgeFile() const
{
	return thePrefs.DoPartiallyPurgeOldKnownFiles() && m_timeLastSeen > 0
		&& time(NULL) > m_timeLastSeen && time(NULL) - m_timeLastSeen > (thePrefs.GetKnownMetDays() == 0 ? OLDFILES_PARTIALLYPURGE : DAY2S(thePrefs.GetKnownMetDays()));
}

bool CKnownFile::ShouldCompletelyPurgeFile() const
{
	return thePrefs.GetCompletlyPurgeOldKnownFiles() && m_timeLastSeen > 0
		&& time(NULL) > m_timeLastSeen && time(NULL) - m_timeLastSeen > (thePrefs.GetKnownMetDays() == 0 ? OLDFILES_PARTIALLYPURGE : DAY2S(thePrefs.GetKnownMetDays()));
}
